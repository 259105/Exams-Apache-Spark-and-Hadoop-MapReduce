...
public class SparkDriver {
	...
	public static void main(String[] args) {
		...
		JavaPairRDD<Tuple2<String, String>, Tuple2<Integer, Integer>> appUserInstall = actionsRDD.filter(line -> {
			String[] fields = line.split(",");
			String action = fields[3];
			...
			if (date.compareTo("2021/12/31") > 0)
				return new Tuple2<>(new Tuple2<>(appId, userId), new Tuple2<Integer, Integer>(1, 0));
			else
				return new Tuple2<>(new Tuple2<>(appId, userId), new Tuple2<Integer, Integer>(0, 1));
		});

		// Count for each key userId, appId how main installations after 2021/12/31
		// and how many installations before 2022/01/01
		JavaPairRDD<Tuple2<String, String>, Tuple2<Integer, Integer>> appUserTotInstall = appUserInstall
				.reduceByKey((v1, v2) -> new Tuple2<Integer, Integer>(v1._1() + v2._1(), v1._2() + v2._2()));
		...
		JavaPairRDD<Tuple2<String, String>, Tuple2<Integer, Integer>> appsNewUsersPairRDD = appUserTotInstall
				.filter(pair -> (pair._2()._1()>0 && pair._2()._2()==0));
		

		// Select only the key part
		JavaRDD<Tuple2<String, String>> appsNewUsers = appsNewUsersPairRDD.keys();
		...
		JavaPairRDD<String, Integer> newInstall2022PerApp = appsNewUsers
		...
		JavaRDD<String> res2 = newInstall2022PerApp.filter(t -> t._2().equals(maxInstallations))
				.keys();
